...
func DayNine(filename string) (int, error) {
	...
	for i, v := range numbersValues {
		...
		sum += numbersValues[i].FinalValues[v.Length]
...
func (nv *NumbersValue) AddFinalNumbers() (int, error) {
	...
	if nv.Length > 1 {
		...
		added = tempsum + nv.GetLastOriginalValue()

		// if nv.NextNumbersValue == nil {
		// 	belowValue = 0
		// 	added = nv.GetLastOriginalValue()
		// } else {
		// 	belowValue = nv.NextNumbersValue.GetLastOriginalValue()
		// 	added = belowValue + nv.GetLastOriginalValue()
		// }

		nv.FinalValues = nv.OriginalValues
